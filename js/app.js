...
jQuery(document).ready(function($) {
    ...
    // OffLoad animations
    $('#listMenu a, #blogList a, #topBarIconBackList').not(".do-not-steal").click(function(e) {
	    e.preventDefault();
	    var anchor = $(this), h;
	    h = anchor.attr('href');
	...
	if ("ontouchstart" in document.documentElement) {
		...
		$('.ShareDialogTrigger').bind('touchstart touchon', function() {
			if(shareToggling){
                var elem = $(this).next();
				elem.show(0).addClass('activeState');
				shareToggling = false;
                setTimeout( function() { 
                elem.removeClass('activeState').delay(400).hide(0);
				shareToggling = true;
                }, 5000);
			}else{
